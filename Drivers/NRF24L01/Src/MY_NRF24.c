...
Written by:				Mohamed Yaqoob (MYaqoobEmbedded YouTube
Channel) Date Written:			10/11/2018 Last modified:
-/-
Description:			This is an STM32 device driver library for the
NRF24L01 Nordic Radio transceiver, using STM HAL libraries

References:				This library was written based on the
Arduino NRF24 Open-Source library by J. Coliz and the NRF24 datasheet
...
static bool
    ack_payload_available; /**< Whether there is an ack payload waiting */
...
// 9. Flush Tx buffer
void NRF24_flush_tx(void) { NRF24_write_register(CMD_FLUSH_TX, 0xFF); }
// 10. Flush Rx buffer
void NRF24_flush_rx(void) { NRF24_write_register(CMD_FLUSH_RX, 0xFF); }
...
void NRF24_startListening(void) {
    ...
    NRF24_write_register(
        REG_CONFIG, NRF24_read_register(REG_CONFIG) | (1UL << 1) | (1UL << 0));
...
bool NRF24_available(void) { return NRF24_availablePipe(NULL); }
...
// 18. Open Tx pipe for writing (Cannot perform this while Listenning, has to
// call NRF24_stopListening)
...
// 23. Get payload size
uint8_t NRF24_getPayloadSize(void) { return payload_size; }
...
void NRF24_enableDynamicPayloads(void) {
    ...
    if (!NRF24_read_register(REG_FEATURE)) {
        NRF24_ACTIVATE_cmd();
        NRF24_write_register(
            REG_FEATURE, NRF24_read_register(REG_FEATURE) | _BV(BIT_EN_DPL));
    }
    // Enable Dynamic payload on all pipes
    NRF24_write_register(REG_DYNPD, NRF24_read_register(REG_DYNPD) |
                                        _BV(BIT_DPL_P5) | _BV(BIT_DPL_P4) |
                                        _BV(BIT_DPL_P3) | _BV(BIT_DPL_P2) |
                                        _BV(BIT_DPL_P1) | _BV(BIT_DPL_P0));
...
bool NRF24_isNRF_Plus(void) { return p_variant; }
...
rf24_datarate_e NRF24_getDataRate(void) {
    ...
    uint8_t dr =
        NRF24_read_register(REG_RF_SETUP) & (_BV(RF_DR_LOW) | _BV(RF_DR_HIGH));
...
void NRF24_setCRCLength(rf24_crclength_e length) {
    uint8_t config =
        NRF24_read_register(REG_CONFIG) & ~(_BV(BIT_CRCO) | _BV(BIT_EN_CRC));
...
// 39. Check if data are available and on which pipe (Use this for multiple rx
// pipes)
...
// 44. Test if there is a carrier on the previous listenning period (useful to
// check for intereference)
bool NRF24_testCarrier(void) { return NRF24_read_register(REG_CD) & 1; }
// 45. Test if a signal carrier exists (=> -64dB), only for NRF24L01+
bool NRF24_testRPD(void) { return NRF24_read_register(REG_RPD) & 1; }
...
uint8_t NRF24_GetAckPayloadSize(void) { return ack_payload_length; }
